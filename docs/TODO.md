...
- `docs/system-requirements/` にはP0ドラフト6文書があるが未承認である。追跡対象の
  テスト、CI、`runs/` は未作成である。
...
- [ ] MVPの入力を人間が指定した1銘柄とし、銘柄識別子と市場を
  定義する。中期・長期を既定の投資評価期間とし、単一期間の指定を必須にしない。
  調査基準時刻はタスク受付時の現在時刻からシステムが固定し、利用者入力にしない。
- [ ] MVPでは自動の一次スクリーニングとLLM簡易調査を実行せず、指定銘柄をCodex・Claudeの詳細解析へ渡す境界を定義する。
- [ ] 財務、バリュエーション、事業、テクニカル、リスク・反証の5観点について、MVPで必要な入力と評価方針を定義する。
- [ ] 財務、株価、開示、ニュースのデータソース、利用規約、料金、認証、再配布可否、
  外部Agent提供者への処理目的の移送可否、鮮度、対象期間、欠損・不一致の扱いを定義する。
- [ ] 人間の運用者が、採用するデータソースとWebサービスごとに必要なAPIキー・認証情報を
  洗い出す。採用決定後に必要なキーを取得し、保管先、実行環境への注入方法、アクセス権、
  更新・失効、漏えい時の対応を決定する。秘密値はGit、文書、ログ、Agent入力へ保存しない。
  - [ ] `EDINET API Version 2`の利用登録を行い、日本の法定開示取得用APIキーを取得する。
  - [ ] `yfinance`とYahoo FinanceデータはAPIキー取得対象外として管理し、個人利用条件、
    保存・再配布、外部Agent提供者への送信、ライブラリ版、取得頻度を設定する。
  - [ ] 日本銀行時系列統計データ検索APIとSEC `data.sec.gov`もAPIキー取得対象外として
    管理し、それぞれの出所表示、User-Agent、アクセス頻度などの利用条件を設定する。
- [ ] モデル選択、独立性、一次レビュー合格条件、限定再調査、争点重要度、追加監査の起動条件、監査結果の適用決定表、人間へのエスカレーションを定義する。Antigravityは通常作業者や多数決の第三票にしない。
- [ ] 後段分析の4段階評価、評価可否と、再調査、追加監査、人間判断待ち、失敗を
  含む実行状態を別の概念として定義する。
- [ ] CLI操作、タイムアウト、再試行、中断、再開、再実行、利用上限、障害時フォールバック、認証方式を定義する。
- [ ] `runs/<task-id>/`、`reports/agent-reports/`、`reports/finalized-reports/` の正本、移送条件、
  MVPでの全量保持、権限を定義する。
...
- [ ] `task_id`、システムが固定したタイムゾーン付き調査基準時刻、市場、対象銘柄、
  中期・長期を既定とする投資評価期間、評価Policy Version、制約を持つ詳細解析タスク契約を
  作成する。通常実行では単一期間の指定を必須にせず、利用者による過去・未来の
  調査基準時刻指定を受け付けない。
...
- [ ] 実行状態、再調査、監査、人間判断、解析完了、最終化、中断、取消、失敗の
...
- [ ] 日本株・米国株の市場データを取得する`yfinance`アダプターと、共通取得インターフェースを実装する。
- [ ] 日本株・米国株の代表fixtureで、銘柄識別、最低3年の日次価格・出来高、調整前後価格、
  配当・株式分割、通貨、タイムゾーン、欠損、取得失敗を検証する。
...
- [ ] 各実行でオーケストレーターと異なるモデル系統の一次レビューワーを1つ割り当て、
  ...
  10往復を使用量分析の参考値として、変更内容、解消・未解決指摘、使用量とともに記録する。
- [ ] 限定再調査と合議往復を別々に計数し、限定再調査の上限、争点分類、重要度ゲート、
...
- [ ] 対象銘柄について、中期・長期の投資仮説、財務、事業、バリュエーション、期間別評価、
  ...
  期間横断要約、エントリー・撤退参考情報、調査基準時刻、データ鮮度を含むJSON出力を
  実装する。
...
- [ ] 解析完了と人間による最終化承認を別状態として実装し、対象成果物のハッシュを照合する。
- [ ] 実行状態にかかわらず、原データ、派生データ、中間・最終成果物、ログ、
  参照用コピー、バックアップを全量保持する。
