...
- [ ] `Antigravity` と `Antigrabity` のどちらを正式表記とするか決定する。
  本文、スクリプト名、設定値、アダプター名、Mermaid ID、成果物名を一貫して更新する。
- [ ] オーケストレーターと異なるモデル系統が一次レビューを1回担当する方針に、README、構想、アーキテクチャの説明を統一する。
- [ ] プロジェクト名、Pythonパッケージ名、説明、配布有無、ライセンス方針を決定し、`pyproject.toml` のテンプレート値を置き換える。
- [ ] Python 3.14を正式な対応バージョンとするか、`jupyter` を開発依存関係に含めるかを決定する。
- [ ] ローカルの `uv`、Docker、または両方のどれを標準開発環境とするか決定する。
- [ ] 選択した開発環境に合わせ、`scripts/pre-commit/` が存在しない `docker/run-docker.sh` を呼び出さず実行できるようにする。
- [ ] `scripts/pre-commit/install-githooks.sh` のリポジトリルート移動と、依存関係の導入案内をプロジェクト方針に合わせる。
- [ ] テスト未作成時の終了コード5の扱いをローカルとDockerで統一し、実装開始後はテスト0件を成功扱いしない方針に切り替える。
- [ ] `shellcheck` と `shfmt` の導入方法とバージョン管理方法を定義する。
- [ ] ロックファイルと再現可能なセットアップ手順を追加する。
- [ ] READMEの現在状態、構成、セットアップ、品質確認を実ファイルに合わせて更新する。
- [ ] `docs/agent-reports/repository-overview.md` を追跡対象にするか生成物として無視するか決定し、採用した方針に合わせて概要を更新する。
...
- [ ] `src/` 配下にプロジェクト固有のPythonパッケージを作成し、最小のCLIエントリーポイントを `pyproject.toml` に登録する。
...
- [ ] Dockerを標準環境に含める場合だけ、Dockerfile、Compose、`docker/run-docker.sh`、必要に応じてdevcontainerを作成する。
...
- [ ] オーケストレーターと異なるモデル系統による一次レビューと、指摘ごとの応答管理を実装する。
- [ ] 限定再調査、回数上限、争点分類、重要度ゲート、追加監査を起動しなかった理由の記録を実装する。
- [ ] 正式名称決定後の第三者監査アダプターを、読み取り専用・外部検索無効・争点限定で実装する。
