...
- [ ] MVPの入力を人間が指定した少数銘柄とし、銘柄識別子、市場、投資期間、
  タイムゾーン付き調査基準時刻、指定可能件数を定義する。
...
- [ ] `task_id`、タイムゾーン付き調査基準時刻、市場、具体的な銘柄一覧、投資期間、
  評価Policy Version、制約を持つ詳細解析タスク契約を作成する。
...
- [ ] 指定銘柄ごとに、投資仮説、財務、事業、バリュエーション、テクニカル、評価、
  評価可否、確信度、重要な根拠、リスク、反証条件、監視条件、欠損、モデル間不一致、
  調査基準時刻、データ鮮度を含むJSON出力を実装する。
...
- [ ] 保持・削除ではタスクルート外への移動を拒否し、削除計画、dry-run、部分失敗、
  参照用コピーとバックアップの結果を検証する。
