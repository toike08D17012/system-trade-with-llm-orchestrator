...
- [ ] `docs/system-requirements/` に、文書の状態、承認者、変更手順、設計文書との責務分担と
...
- [ ] 外部リクエストを共有Coordinatorへ集約し、provider別キュー、rate limit、cooldown、
  cache、single-flight、実行時lease、再開時checkpoint、監査ログの要件を定義する。
- [ ] `runs/<task-id>/`、`reports/agent-reports/`、`reports/finalized-reports/` の正本、移送条件、
  MVPでの全量保持、権限を定義する。
- [ ] 秘密情報除去、プロンプトインジェクション対策、外部コンテンツとAgent指示の分離を定義する。
- [ ] 人間向けの承認・最終文書を日本語のMarkdown、Agent用および機械可読成果物の
...
- [ ] 外部リクエストについて、論理要求と物理要求、request fingerprint、source profile、
  適用したrate gate、待機・再試行、cache hit、single-flight、使用量を追跡する契約を作成する。
...
- [ ] Coordinatorについて、論理要求と物理要求、provider別待機、適用gate、cooldown、
  retry、cache hit、single-flight、拒否、安全停止、実行時leaseを監査・計測できるようにする。
