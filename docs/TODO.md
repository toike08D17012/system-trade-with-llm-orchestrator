...
- DukascopyのUTC Bid日足終値への移行計画は承認済み。公式利用条件の
  確認事項を提示後、利用者が利用を明示した。
...
- `src/stock_research_llm_orchestrator/` には契約基盤とoffline設定検証CLIに加え、
  市場証拠の準備・再検証・固定期間の価格受入、Dukascopy為替取得と日付ラベル結合を行う内部API・CLIがある。
...
- [x] `task_id`、タイムゾーン付き`task_accepted_at`、市場、対象銘柄、中期・長期を既定とする
  投資評価期間、評価Policy Version、制約を持つ詳細解析タスク契約を作成する。通常実行では
  単一期間の指定を必須にせず、利用可能情報を任意時刻で打ち切る指定を受け付けない。
- [x] `evidence_set_version`、`evidence_frozen_at`、`freshness_checked_at`、
  `analysis_completed_at`を持つ証拠集合・実行契約を作成し、重要な新規情報の採用時に
  影響成果物を無効化して再実行できるようにする。
- [x] 銘柄識別子、名称、通貨、セクターを表す銘柄情報の契約を作成する。
- [x] 原データ、出典メタデータ、正規化済みデータ、計算済み指標を分離し、`evidence_id`、取得日時、対象期間、参照先、ハッシュ、データ版、計算ロジック版を追跡できる契約を作成する。
- [x] 指定銘柄、調査条件、共通証拠、出典メタデータを作業者へ渡す `research_context` を定義する。
- [x] Agent別の検索語、検索機能、実行日時、返却URL、スニペット、候補資料、支持・反証対象を
  保存する検索記録と、候補資料の検証・採否を保存する契約を作成する。
- [x] 検証済み追加証拠の和集合と発見元を保持し、相手の暫定結論を含めず両作業者へ
  差し戻す共通証拠更新契約を作成する。
...
- [x] 統合結果と、期間別の合意点、相違点、欠損、反証条件、暫定判定、期間横断要約の
  契約を作成する。
- [x] 一次レビュー指摘と指摘ごとの応答契約を作成する。
- [x] findingの説明と分類、重要度への異議、materiality、争点化、1セッション1争点、
  中立な争点パケット、追加監査結果、適用決定表および論理監査回数の方針をADR-0004と
  system requirements 03版1.1へ記録する。
- [x] ADR-0004に従い、finding、response、classification、materiality、dispute、audit、
  監査起動判定および人間判断入力の機械可読契約を作成する。
- [x] 実行状態、再調査、監査、人間判断、解析完了、中断、取消、失敗の
  状態遷移と `manifest` を定義する。
- [x] 最終判定、日本語の人間向けMarkdown、英語の機械可読JSON、ログ、成果物間の
  参照契約と、人間向け文書のタスク単位言語上書き設定を作成する。
- [x] 各契約について、正常、欠損、矛盾、無効な証拠参照、スキーマ違反、旧バージョンのfixtureを作成する。
- [x] `agent-sources/` にオーケストレーター、Codex作業者、Claude作業者、一次レビューワー、条件付き第三者監査の役割別指示を作成する。
- [x] `agent-sources/` に証拠参照、事実・推論・仮説、反証、欠損、出力検証、安全境界の共通指示を作成する。
- [x] 指示原本の配布方法、バージョン、実際に適用した設定を `manifest` に記録する方法を定義する。
- [x] 外部リクエストについて、論理要求と物理要求、request fingerprint、source profile、
  適用したrate gate、待機、エラー、cooldown、cache hit、single-flight、使用量を追跡する契約を作成する。

### P2: 最小開発基盤

- [x] 最小のCLIエントリーポイントを `pyproject.toml` に登録する。
- [x] 設定読み込み、終了コード、エラー表示、ログ初期化の共通基盤を作成する。
- [x] 追跡対象の `tests/` を作成し、パッケージ読込、CLIの `--help`、設定検証の最小テストを追加する。
- [x] 最初のテストスイート追加と同時に、Pytestの終了コード5を成功扱いする暫定処理を削除する。
- [x] CIでロック済み依存関係を導入し、format check、lint、型チェック、テスト、Shell検証を読み取り専用で実行する。
...
- [ ] 東証上場内国株の代表fixtureで、銘柄識別、最低3年の日次価格・出来高、調整前後価格、
  ...
  対応市場の拡張時に追加する。
- [ ] DukascopyのUTC Bid日足USD/JPYを取得し、
  同じ日付ラベルの東証終値と結合してドル換算系列を計算する。片方が欠損・未公表の場合は
  別日の為替値で補完せず、ドル換算値を未確定または欠損とする。
  - [x] 旧BOJ v2送信制限を承認し、単発取得・raw公開後の論理確定・オフライン同日結合を実装する。
    固定fixtureで来歴・欠損・Decimal計算・改変拒否を検証する。
  - [x] 初回GET 1回の応答を保存し、分類ラベルの検証修正後に通信なしで再検証する。
    元の論理失敗を保持してFX証拠を受け入れ、価格731日中729日を換算した。
    [実確認結果](decision-requests/2026-09-26-boj-fx-evidence-acceptance-outcome.md)に保存先・hashを記録した。
  - [x] Dukascopyへ移行し、4 GETでUTC Bid日足を取得した。9/24・9/25も含め731/731日を換算した。
    [移行の実確認結果](decision-requests/2026-09-26-dukascopy-fx-evidence-acceptance-outcome.md)に来歴を記録した。
  - [ ] 通常実行の必要終端日・証拠集合凍結へ接続する。
...
- [ ] 決定論的なHTTP取得を共有Coordinator経由に限定し、provider別制限、共有cooldown、
  ...
  - [x] ADR-0005 Phase 6Bとして、raw forward repair、pre-rename candidate破棄、committed integrity検証、
    logical requestからのreference解決、duplicate/replayのidempotencyを追加する。
  - [x] ADR-0005 Phase 7共通基盤として、credential-free canonical intent、bounded exact response、
...
- [ ] タスク、指定銘柄、原データ、正規化済みデータ、計算済み指標、出典メタデータ、`manifest` を採用済みの実行ディレクトリへ原子的に保存する。
  - [x] 検証済みbytes一式を専用の内部準備directoryへ同一filesystemのrenameで原子的に公開し、
    既存宛先を上書きせず、相対pathと実bytesのSHA-256をreceiptとして返す基盤を追加する。
  - [ ] 公開artifactの参照とmanifest状態を確定し、完全な証拠一式を採用済み実行directoryへ保存する。
