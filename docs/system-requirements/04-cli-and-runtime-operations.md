...
| 開始 | 新しい`task_id`を発行し、入力と実行設定を固定して開始する |
...
| 取消 | タスクを再開しない終端状態へ移し、保持期間の起点と取消理由を記録する |
| 再開 | 同じ`task_id`の検証済みチェックポイントから未完了工程だけを続行する |
| 再実行 | 新しい`task_id`を発行し、親タスク、再利用した入力・証拠、変更設定を記録する |
...
- `Suspended` と人間判断待ちは非終端状態とし、自動削除の保持期間を開始しない。
  取消は権限を持つ人間または承認済みの放棄Policyだけが実行できる。

## 6. 利用上限

タスク開始前に、少なくとも次のハード上限を設定する。

- タスク全体と工程ごとの時間
- 通常作業者、レビュー、再調査、追加監査の実行回数
- 総入力・出力トークン
- データAPI呼び出し数
- 外部データ費用とAgent利用費用の合計
- 1タスクの銘柄数と争点数

上限の80%で警告する。新規外部呼び出し前に、その呼び出しで許可する最大トークン、
API呼び出し、時間、金額を予約し、現在使用量と予約量の合計がハード上限を超える場合は
開始しない。完了後に予約量を権威ある実測値または保守的な推定値へ精算し、両者を
区別して記録する。承認可能な最大量を事前に制限・推定できない提供者は、
ハード上限を要求する実行で使用しない。停止時は、利用量、完了工程、未処理事項、
再開に必要な追加上限を記録する。
上限を実行中に自動拡張しない。
