...
7. 外部へ直接送信する決定論的なデータ取得と候補URLの検証取得は、共有
   `RequestCoordinator`を経由する。source adapterまたはretry処理による直接送信を許可しない。
...
- Agent内蔵Web検索は、Codex系・Claude系、オーケストレーター、一次レビューワー、
  ...
  task・role・provider accountごとの検索利用枠を予約・制御する。provider単位の物理request制御が
...
`rejected`となったソースの既存証拠は自動削除しないが、新しいAgent送信、解析再開、最終化、
...
| 送信制御 | `max_concurrency`、`min_interval`、`rate`、`window`、`burst`、`daily_hard_limit`を定める |
| 再利用 | 利用条件、認証scope、鮮度、対象期間を含む`cache_policy`と`batch_policy`を定める |
| 再試行 | retry対象、最大attempt、最大待機時間を含む`retry_policy`を定める |
...
不足を明示する。必須データの不足が上限内の限定再調査で解消可能な間は、影響期間を
`evaluable`の`再調査`とする。取得不能、利用条件違反、または再調査上限到達により
必須データを確保できない場合だけ、影響期間を`not_evaluable`とする。任意データの不足だけが
